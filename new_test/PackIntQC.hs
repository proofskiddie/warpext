...
propPackInt :: Int -> Bool
propPackInt n = packIntegralPure (abs n) == packIntegral (abs n)
mainPackInt = quickCheck propPackInt

{-# SPECIALIZE packIntegralPure :: Int -> ByteString #-}
{-# SPECIALIZE packIntegralPure :: Integer -> ByteString #-}

packStatusPure :: H.Status -> ByteString
packStatusPure status = foldr B.cons B.empty [r2,r1,r0]
  where
...
propPackStatus :: H.Status -> ByteString -> Bool
propPackStatus s b = packStatusPure s b == packStatus s b
mainPackStatus = quickCheck propPackStatus
